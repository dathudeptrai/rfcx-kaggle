...
class BalancedMelSamplerPL_old(tf.keras.utils.Sequence):
    def __init__(
        self,
        dict_data,
        pl_data,
...
class BalancedMelSamplerPL(tf.keras.utils.Sequence):
    ...
    def _getitem(
        ...
        return batch_x_aug, categorical_batch_y, batch_y_seg_aug, batch_r, mix
    ...
    def __getitem__(self, index):
        ...
        if np.random.random() > self.pl_prop:
            ...
            is_cutmix = np.array(True)
            batch_r = np.array([''] * len(batch_x_aug_tp))
