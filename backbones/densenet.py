...
def transition_block(x, reduction, name, use_mixstyle=False):
    """A transition block.
    ...
    if use_mixstyle:
        x = MixStyle(p=0.5, alpha=0.1, name=name + "_mixstyle")(x)
    return x
...
def DenseNet(
    ...
    x = transition_block(x, 0.5, name="pool2", use_mixstyle=use_mixstyle)
    x = dense_block(x, blocks[1], name="conv3")
    x = transition_block(x, 0.5, name="pool3", use_mixstyle=use_mixstyle)
    x = dense_block(x, blocks[2], name="conv4")
    x = transition_block(x, 0.5, name="pool4", use_mixstyle=False)
